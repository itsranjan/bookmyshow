package com.main.bookmyshow.Services;

import com.main.bookmyshow.Dtos.RequestDtos.TheaterEntryDto;
import com.main.bookmyshow.Dtos.RequestDtos.TheaterSeatEntryDto;
import com.main.bookmyshow.Enums.SeatType;
import com.main.bookmyshow.Exceptions.ScreenAlreadyAdded;
import com.main.bookmyshow.Exceptions.ScreenNumberOutOfBound;
import com.main.bookmyshow.Exceptions.TheaterIsNotPresentOnThisAddress;
import com.main.bookmyshow.Exceptions.TheaterIsPresentOnThatAddress;
import com.main.bookmyshow.Models.Show;
import com.main.bookmyshow.Models.Theater;
import com.main.bookmyshow.Models.TheaterSeat;
import com.main.bookmyshow.Repositories.TheaterRepository;
import com.main.bookmyshow.Transformers.TheaterTransformer;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import java.rmi.AlreadyBoundException;
import java.util.List;

@Service
public class TheaterService {

    @Autowired
    private TheaterRepository theaterRepository;
    public String addTheater(TheaterEntryDto theaterEntryDto) throws TheaterIsPresentOnThatAddress{
        if(theaterRepository.findByAddress(theaterEntryDto.getAddress()) != null) {
            throw new TheaterIsPresentOnThatAddress();
        }
        Theater theater = TheaterTransformer.theaterDtoToTheater(theaterEntryDto);

        theaterRepository.save(theater);
        return "Theater has been saved Successfully";
    }

    /*public String addTheaterScreen(TheaterScreenEntryDto entryDto){
        if(theaterRepository.findByAddress(entryDto.getAddress()) ==null){
            throw new TheaterIsNotPresentOnThisAddress();
        }
        String address = entryDto.getAddress();
        Theater theater = theaterRepository.findByAddress(address);
        Integer total_screen = theater.getNoofscreen();
        Integer screen_no = entryDto.getNoofscreen();

        if(!(screen_no>0 && screen_no<=total_screen)){
            throw new ScreenNumberOutOfBound();
        }
        List<TheaterScreen> theaterscreen = theater.getTheaterScreen();
        TheaterScreen theaterScreen = new TheaterScreen();
        theaterScreen.setScreenNo(screen_no);
        theaterScreen.setAddress(entryDto.getAddress());
        theaterScreen.setTheater(theater);
        theaterscreen.add(theaterScreen);
        theaterRepository.save(theater);

        return "Screen added to your theater";

    }*/
    public String addTheaterSeat(TheaterSeatEntryDto entryDto) throws TheaterIsNotPresentOnThisAddress,ScreenNumberOutOfBound{
        if(theaterRepository.findByAddress(entryDto.getAddress()) == null) {
            throw new TheaterIsNotPresentOnThisAddress();
        }
        Integer screen_no = entryDto.getScreenno();
        Integer noOfSeatsInRow = entryDto.getNoOfSeatInRow();
        Integer noOfPremiumSeats = entryDto.getNoOfPremiumSeat();
        Integer noOfClassicSeat = entryDto.getNoOfClassicSeat();
        String address = entryDto.getAddress();

        Theater theater = theaterRepository.findByAddress(address);
        Integer total_screen = theater.getNoofscreen();
        List<TheaterSeat> seatList = theater.getTheaterSeatList();

        for(TheaterSeat seat:seatList){
            if(seat.getScreenno().equals(entryDto.getScreenno())){
                throw new ScreenAlreadyAdded();
            }
        }

        if(screen_no<0 || screen_no>total_screen){
            throw new ScreenNumberOutOfBound();
        }


        int counter = 1;
        int fill = 0;
        char ch = 'A';

        for(int i = 1; i <= noOfClassicSeat; i++) {
            String seatNo = Integer.toString(counter)+ch;

            ch++;
            fill++;
            if(fill == noOfSeatsInRow) {
                fill = 0;
                counter++;
                ch = 'A';
            }

            TheaterSeat theaterSeat = new TheaterSeat();
            theaterSeat.setScreenno(screen_no);
            theaterSeat.setSeatNo(seatNo);
            theaterSeat.setSeatType(SeatType.CLASSIC);
            theaterSeat.setTheater(theater);
            seatList.add(theaterSeat);
        }

        for(int i = 1; i <= noOfPremiumSeats; i++) {
            String seatNo = Integer.toString(counter)+ch;

            ch++;
            fill++;
            if(fill == noOfSeatsInRow) {
                fill = 0;
                counter++;
                ch = 'A';
            }

            TheaterSeat theaterSeat = new TheaterSeat();
            theaterSeat.setScreenno(screen_no);
            theaterSeat.setSeatNo(seatNo);
            theaterSeat.setSeatType(SeatType.PREMIUM);
            theaterSeat.setTheater(theater);
            seatList.add(theaterSeat);
        }

        theaterRepository.save(theater);

        return "Theater Seats have been added successfully";
    }
}
